...
个人 LeetCode 刷题笔记与 Java 实现合集，侧重**思路说明**与**逐行注释**，非常适合对编程语言还不熟练的小白或是练题萌新，想练习数据结构与算法。
‼️ACMer请移步b站跟大佬们学时间空间最优的模板，不要在这里浪费时间。
保留 **ACM 风格** 的 `main` 输入输出，便于本地运行验证
对于链表，提供多种输入模式的buildList函数，printList函数
对于树，提供层序遍历的buildTree和printTree函数。
